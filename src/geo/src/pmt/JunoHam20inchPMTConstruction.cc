...
#include <RAT/JunoHam20inchPMTConstruction.hh>
#include <algorithm>

namespace RAT {

JunoHam20inchPMTConstruction::JunoHam20inchPMTConstruction(DBLinkPtr table, G4LogicalVolume *mother)
    : PMTConstruction("juno") {
    //: PMTConstruction("revolution") {
  G4cout << "Starting to generate JunoHam20inchPMTConstruction..." << G4endl;
  ...
  Log::Assert(fParams.rEdge.size() == fParams.zEdge.size(),
              "JunoHam20inchPMTConstruction: " + pmt_model + " must have equal length rho_edge, z_edge arrays");
  Log::Assert(fParams.rInner.size() == fParams.zInner.size(),
              "JunoHam20inchPMTConstruction: " + pmt_model + " must have equal length rho_inner, z_inner arrays");
  ...
  Log::Assert(fParams.exterior, "JunoHam20inchPMTConstruction: " + pmt_model + " has an invalid exterior material");
  Log::Assert(fParams.glass, "JunoHam20inchPMTConstruction: " + pmt_model + " has an invalid glass material");
  Log::Assert(fParams.vacuum, "JunoHam20inchPMTConstruction: " + pmt_model + " has an invalid vacuum material");
  Log::Assert(fParams.dynode, "JunoHam20inchPMTConstruction: " + pmt_model + " has an invalid dynode material");
  Log::Assert(fParams.photocathode,
              "JunoHam20inchPMTConstruction: " + pmt_model + " has an invalid photocathode material");
  Log::Assert(fParams.mirror, "JunoHam20inchPMTConstruction: " + pmt_model + " has an invalid mirror surface");
  Log::Assert(fParams.dynode_surface, "JunoHam20inchPMTConstruction: " + pmt_model + " has an invalid dynode material");
...
G4LogicalVolume *JunoHam20inchPMTConstruction::BuildVolume(const std::string &prefix) {
  if (body_log) {
    return body_log;
  }
  ...
  if (inner_equator_idx == 0) {
    Log::Die("JunoHam20inchPMTConstruction: inner equator not found");
  }
...
G4VSolid *JunoHam20inchPMTConstruction::BuildSolid(const std::string &_name) {
...
G4PVPlacement *JunoHam20inchPMTConstruction::PlacePMT(G4RotationMatrix *pmtrot, G4ThreeVector pmtpos,
